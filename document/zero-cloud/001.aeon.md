...
&ensp;&ensp;&ensp;&ensp;安装完成后的问题：

1. **/usr/local/bin/crio No such file or directory**
	...
	```
	这个问题是因为crio.service中使用了`/usr/local/bin/crio`作为执行路径，而安装路径在`/usr/bin/crio`，所以需额外创建一个软链接：
	```shell
	ln -s /usr/bin/crio /usr/local/bin/crio
	```
2. 残留了两个状态检查时遇到的问题（**该问题似乎不影响crio运行，暂时先保留，后期遇到问题再解决**）：
...
&ensp;&ensp;&ensp;&ensp;接下来的确是`kubeadm`的高光时刻，几乎全程使用它来配置K8S集群。

## 4. 小结

&ensp;&ensp;&ensp;&ensp;由于引用解析在Gitbook在线版有问题，此处截图一份引用内容：

![](./_image/2022-09-13/20220913234931.png)

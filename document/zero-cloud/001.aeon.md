...
&ensp;&ensp;&ensp;&ensp;**CentOS Stream究竟能否用于生产**？CentOS Stream出现后[^22]，首先发布顺序有了变化：
...
&ensp;&ensp;&ensp;&ensp;最后使用下边命令安装Podman和CRI-O配合做镜像管理，后续步骤会配合做镜像拉取。

```shell
yum install podman
```

#### 3.2.3. 三剑客：kubeadm/kubelet/kubectl
...
&ensp;&ensp;&ensp;&ensp;残留了两个状态检查时遇到的问题，**该问题似乎不影响crio运行，暂时先保留，后期遇到问题再解决**。
...
&ensp;&ensp;&ensp;&ensp;`--apiserver-advertise-address`可用于为控制平面节点的AIP Server设置广播地址，`--control-plane-endpoint`可用于为所有控制平面节点设置共享端点，它允许IP地址和可映射到IP地址的DNS名称，您可以和网络管理员联系，以评估有关此类映射的可能解决方案。例如：

```shell
192.168.0.102 cluster-endpoint
```

&ensp;&ensp;&ensp;&ensp;其中`192.168.0.102`是此节点的IP地址，`cluster-endpoint`是映射到该IP的自定义DNS名称，这将允许您将`--control-plane-endpoint=cluster-endpoint`传递给`kubeadm init`，并将相同的DNS名称传递给`kubeadm join`。稍候您还可以修改`cluster-endpoint`以指向高可用性方案中的负载均衡器的地址。`kubeadm`不支持将没有`--control-plane-endpoint`参数的单个控制平面集群转换为高可用集群。

...
1. 先检查Podman中的容器，并停止该容器：

    ```shell
    podman ps               # 查看Podman中容器的状态
    podman stop <ID>        # 停止Podman中的容器（自动移除）
    ```

    ![](./_image/2022-11-17/20221117180800.png)

2. 根据错误信息中的UUID查看目前还存在的文件：

    ```shell
    # 最后一次错误信息的UUID：d3e2cc46-665e-11ed-8e10-fa163ec820ba
    find / -name "*d3e2cc46-665e-11ed-8e10-fa163ec820ba*"
    ```

    ![](./_image/2022-11-17/20221117181934.png)

3. 从上述查询结果可以看到和该UUID相关的目录很多，将这些目录全部查询出来删除掉，保证系统中没有任何和ceph相关的内容就可以了。

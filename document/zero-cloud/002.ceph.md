...
&ensp;&ensp;&ensp;&ensp;本章节使用`cephadm`的方式部署Ceph集群[^3]，您可以访问官方文档：<https://docs.ceph.com/en/pacific/install/>，官方推荐的安装方式如下：
...
&ensp;&ensp;&ensp;&ensp;安装OSD之前，需要在Linux环境中先对磁盘做部分操作，创建对应的扩展分区。
...
1. 查看目前的磁盘分区信息：
    ...
    ![](./_image/2022-11-20/20221120182520.png)

    * `fdisk -l 硬盘名|分区名`命令可直接显示或操作磁盘分区表，输出信息主要包含两部分内容：1）硬盘的基本参数，包括**容量、磁头数、扇区数（Sector）**；2）显示硬盘的分区情况，如果已分区，每行代表一个分区
        * *Device*：分区名
        * *Boot*：是否允许标准DOS PC主引导记录引导该分区（GRUB引导无意义）
        * *Start*：分区起始柱面
        * *End*：分区结束柱面
        * *Blocks*：大小为1k的块的数量
        * *Id*：分区预期用途，82代表交换分区，83表示数据分区
    * `lsblk`命令：一般我们更推荐使用lsblk命令查看系统中所有硬盘信息，该命令会现实除了内存虚拟磁盘外的所有可用块设备信息，可以更直观了解当前系统的硬盘分区和挂载情况。
...
&ensp;&ensp;&ensp;&ensp;言归正传，做完上述步骤之后，我们只是单纯地搞定了`k8s-master`节点，并且只是添加了一个Monitor和一个Manager（**裸集群**），接下来需要将其他组件一一追加到集群中。

1. 在三个节点设置ceph的免密登录

    ```shell
    # 您可以先确认 ceph.pub 文件是否存在，再执行如下命令
    ...
    ![](./_image/2022-11-20/20221120181658.png)
...
[^3]: [CentOS 8下使用cephadm搭建Ceph集群](https://blog.csdn.net/Little_cst/article/details/119839818), 作者：[Little_cst](https://blog.csdn.net/Little_cst)
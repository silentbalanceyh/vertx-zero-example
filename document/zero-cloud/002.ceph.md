...
&ensp;&ensp;&ensp;&ensp;**注意**：如果你用的是阿里云的镜像，建议优先考虑自己把这些内容全部拉去下来，但是最后一个镜像只能从`docker.io`去拉，这个镜像在仓库里是存在的，但是**阿里云无法拉取该镜像**，这个过程有可能会导致你的容器在执行部分初始化等相关操作时失效，如果从**阿里云**拉取，您可能遇到下边的错误信息（最少我**个人的阿里云镜像是拉不到这个镜像的**）：

...
&ensp;&ensp;&ensp;&ensp;此处`initial`的左值表示领域模型中拥有的属性值，右值则是使用`common-jexl3`的解析表达式，入参为：
...
&ensp;&ensp;&ensp;&ensp;其他四个参数此处不解释，`module`是在**双表连接**时使用的关键参数，标准化接口中支持`view`和`module`的完整表结构如下：
...
|PUT|/api/batch/:actor/update|
|DELETE|/api/batch/:actor/delete|
|GET|/api/columns/:actor/full|
|GET|/api/columns/:actor/my|
|PUT|/api/columns/:actor/my|
|POST|/api/:actor/import|
|POST|/api/:actor/export|
|POST|/api/:actor/existing|
|POST|/api/:actor/missing|
